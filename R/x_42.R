...
x_42 <- function (x, ref_ds, ...) {
  ...
  x_align <- x_peak %>%
    ...
    mutate(RTalign = align_rt(rt = RTest,
                              pf = Proteoform,
                              x_ref = x_ref,
                              # The dots are for the loess function arguments.
                              ...)) %>%
    ungroup()
...
align_rt <- function (rt, pf, x_ref, ...) {
  ...
  both <- intersect(x_ref$Proteoform, pf)
  ...
  model_loess <- loess(x_ref$RTest[idx_ref] ~ rt[idx_cur], ...)
  
  # Return the predicted retention times.
  return (predict(object = model_loess, newdata = rt))
