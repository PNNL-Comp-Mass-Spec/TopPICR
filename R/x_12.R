x_12 <- function (x,
  ...
  x_norm <- x %>%
    filter(!isDecoy) %>%
    augment_annotation(fst_norm) # This takes a long time ~1 hour.
  ...
  x_norm <- inner_join(x_norm, prot_len_norm)
  ...
  x_norm <- x_norm %>%
    mutate(Block = str_sub(Dataset, start = 19, end = 19)) %>%
    mutate(PercentCoverage = signif(100 * (Last_AA - First_AA + 1) / ProtLen,2))
...
get_matching_uniprotacc <- function (gene,
  ...
  # The "sequence" comes from the augment_annotation function. The cleanSeq
  # string (which is the "sequence") is created there.
  # names_i is the name of the sequence from fst_i that matches the sequence in
  # the input of get_matching_uniprotacc function. The full name of the sequence
  # is the output of this line
  names_i <- sapply(fst_i, grepl, pattern=sequence, USE.NAMES = T) %>% 
  ...
  out <- data.frame(Gene = gene,
    ...
    # comes from.
    mutate(AnnType = case_when(grepl("^(XXX_)?sp\\|[^-]*-\\d+\\|.*",
                                     names_i) ~ "VarSplic",
                               grepl("^(XXX_)?tr.*",
                                     names_i) ~ "TrEMBL",
                               grepl("^(XXX_)?sp\\|[^-]*\\|.*",
